...
use datalayer_driver::{
    Peer, Coin, CoinSpend, SecretKey, PublicKey, Bytes32, Bytes, Signature,
...
impl Wallet {
    ...
    pub async fn create_key_ownership_signature(&self, nonce: &str) -> Result<String, WalletError> {
        let message = format!("Signing this message to prove ownership of key.\n\nNonce: {}", nonce);
        let private_synthetic_key = self.get_private_synthetic_key().await?;
        
        let signature = sign_message(Bytes::from(message.as_bytes().to_vec()), private_synthetic_key)
    ...
    pub async fn verify_key_ownership_signature(
        ...
        if pk_bytes.len() != 48 {
            return Err(WalletError::CryptoError("Invalid public key length".to_string()));
        }
        ...
        if sig_bytes.len() != 96 {
            return Err(WalletError::CryptoError("Invalid signature length".to_string()));
        }
        ...
        verify_signature(Bytes::from(message.as_bytes().to_vec()), public_key, signature)
            .map_err(|e| WalletError::CryptoError(e.to_string()))
    }
    ...
    pub async fn is_coin_spendable(
        peer: &Peer,
        coin_id: &Bytes32,
    ) -> Result<bool, WalletError> {
    ...
    pub async fn connect_random_peer(
        ...
        connect_random(network, cert_path, key_path).await
    ...
    pub async fn connect_mainnet_peer() -> Result<Peer, WalletError> {
        ...
        let ssl_dir = home_dir.join(".chia").join("mainnet").join("config").join("ssl").join("wallet");
        let cert_path = ssl_dir.join("wallet_node.crt");
        let key_path = ssl_dir.join("wallet_node.key");
        
        Self::connect_random_peer(
            NetworkType::Mainnet,
            cert_path.to_str().ok_or_else(|| WalletError::FileSystemError("Invalid cert path".to_string()))?,
            key_path.to_str().ok_or_else(|| WalletError::FileSystemError("Invalid key path".to_string()))?,
        ).await
    }
    ...
    pub async fn connect_testnet_peer() -> Result<Peer, WalletError> {
        ...
        let ssl_dir = home_dir.join(".chia").join("testnet11").join("config").join("ssl").join("wallet");
        let cert_path = ssl_dir.join("wallet_node.crt");
        let key_path = ssl_dir.join("wallet_node.key");
    ...
    async fn save_wallet_to_keyring(
        ...
        keyring.wallets.insert(wallet_name.to_string(), encrypted_data);
    ...
    fn decrypt_data(encrypted_data: &EncryptedData) -> Result<String, WalletError> {
        ...
        String::from_utf8(plaintext)
            .map_err(|e| WalletError::CryptoError(format!("Failed to convert decrypted data to string: {}", e)))
    }
}
...
mod tests {
    ...
    async fn test_wallet_import() {
        ...
        // Import the wallet
        let imported_mnemonic = Wallet::import_wallet("imported_wallet", Some(test_mnemonic)).await.unwrap();
        
        // Verify the mnemonic matches
        assert_eq!(imported_mnemonic, test_mnemonic);
        
        // Load the wallet and verify mnemonic
        let wallet = Wallet::load(Some("imported_wallet".to_string()), false).await.unwrap();
    ...
    async fn test_wallet_load_with_creation() {
        ...
        let wallet = Wallet::load(Some("auto_created".to_string()), true).await.unwrap();
    ...
    async fn test_key_derivation() {
        ...
        Wallet::import_wallet("key_test", Some(test_mnemonic)).await.unwrap();
        let wallet = Wallet::load(Some("key_test".to_string()), false).await.unwrap();
        ...
        assert_eq!(secret_key_to_public_key(&private_synthetic_key), public_synthetic_key);
        ...
        let wallet2 = Wallet::load(Some("key_test".to_string()), false).await.unwrap();
    ...
    async fn test_address_generation() {
        ...
        Wallet::import_wallet("address_test", Some(test_mnemonic)).await.unwrap();
        let wallet = Wallet::load(Some("address_test".to_string()), false).await.unwrap();
    ...
    async fn test_signature_creation_and_verification() {
        ...
        Wallet::import_wallet("sig_test", Some(test_mnemonic)).await.unwrap();
        let wallet = Wallet::load(Some("sig_test".to_string()), false).await.unwrap();
        ...
        // Verify signature
        let is_valid = Wallet::verify_key_ownership_signature(nonce, &signature, &public_key_hex).await.unwrap();
        assert!(is_valid);
        
        // Test with wrong nonce (should fail)
        let is_valid_wrong = Wallet::verify_key_ownership_signature("wrong_nonce", &signature, &public_key_hex).await.unwrap();
    ...
    async fn test_multiple_wallets() {
        ...
        let w1 = Wallet::load(Some("wallet1".to_string()), false).await.unwrap();
        let w2 = Wallet::load(Some("wallet2".to_string()), false).await.unwrap();
        let w3 = Wallet::load(Some("wallet3".to_string()), false).await.unwrap();
    ...
    async fn test_invalid_signature_verification() {
        ...
        // Create wallet
        let test_mnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon art";
        Wallet::import_wallet("invalid_sig_test", Some(test_mnemonic)).await.unwrap();
        let wallet = Wallet::load(Some("invalid_sig_test".to_string()), false).await.unwrap();
        
        let public_key = wallet.get_public_synthetic_key().await.unwrap();
        let public_key_hex = hex::encode(public_key.to_bytes());
        
        // Test with invalid signature format
        let result = Wallet::verify_key_ownership_signature("nonce", "invalid_hex", &public_key_hex).await;
        assert!(result.is_err());
        
        // Test with wrong signature length
        let short_sig = "deadbeef";
        let result = Wallet::verify_key_ownership_signature("nonce", short_sig, &public_key_hex).await;
        assert!(result.is_err());
        
        // Test with invalid public key
        let result = Wallet::verify_key_ownership_signature("nonce", &"a".repeat(192), "invalid_key").await;
        assert!(result.is_err());
    }
