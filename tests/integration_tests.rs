...
async fn test_full_wallet_lifecycle() {
    ...
    // 2. Load the wallet
    let wallet = Wallet::load(Some("lifecycle_test".to_string()), false).await.unwrap();
    ...
    assert_eq!(datalayer_driver::secret_key_to_public_key(&private_key), public_key);
...
async fn test_wallet_import_and_consistency() {
    ...
    Wallet::import_wallet("consistent1", Some(test_mnemonic)).await.unwrap();
    Wallet::import_wallet("consistent2", Some(test_mnemonic)).await.unwrap();
    
    // Load both wallets
    let wallet1 = Wallet::load(Some("consistent1".to_string()), false).await.unwrap();
    let wallet2 = Wallet::load(Some("consistent2".to_string()), false).await.unwrap();
...
async fn test_multiple_wallet_isolation() {
    ...
    for wallet_name in &wallets_to_create {
        let wallet = Wallet::load(Some(wallet_name.to_string()), false).await.unwrap();
        let address = wallet.get_owner_public_key().await.unwrap();
...
async fn test_signature_verification_edge_cases() {
    ...
    let test_mnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon art";
    Wallet::import_wallet("signature_test", Some(test_mnemonic)).await.unwrap();
    let wallet = Wallet::load(Some("signature_test".to_string()), false).await.unwrap();
    ...
    let result = Wallet::verify_key_ownership_signature(nonce, &signature, &public_key_hex).await.unwrap();
    assert!(result);
    
    // Test 2: Wrong nonce should fail
    let result = Wallet::verify_key_ownership_signature("wrong_nonce", &signature, &public_key_hex).await.unwrap();
    assert!(!result);
    
    // Test 3: Invalid signature hex should error
    let result = Wallet::verify_key_ownership_signature(nonce, "invalid_hex_signature", &public_key_hex).await;
    assert!(result.is_err());
    
    // Test 4: Wrong signature length should error
    let short_signature = "deadbeef";
    let result = Wallet::verify_key_ownership_signature(nonce, short_signature, &public_key_hex).await;
    assert!(result.is_err());
    
    // Test 5: Invalid public key should error
    let result = Wallet::verify_key_ownership_signature(nonce, &signature, "invalid_public_key").await;
    assert!(result.is_err());
...
async fn test_address_encoding_edge_cases() {
    ...
    let _temp_dir = setup_integration_test_env();
    let test_mnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon art";
    Wallet::import_wallet("address_edge_test", Some(test_mnemonic)).await.unwrap();
    let wallet = Wallet::load(Some("address_edge_test".to_string()), false).await.unwrap();
...
async fn test_encryption_robustness() {
    ...
    for test_data in test_cases {
        ...
        if test_data.len() > 10 {
            ...
            Wallet::import_wallet(&wallet_name, Some(real_mnemonic)).await.unwrap();
            
            // Load and verify
            let wallet = Wallet::load(Some(wallet_name.clone()), false).await.unwrap();
...
async fn test_concurrent_wallet_operations() {
    ...
    for name in wallet_names {
        let handle = tokio::spawn(async move {
            ...
            let signature = wallet.create_key_ownership_signature("concurrent_test").await.unwrap();
